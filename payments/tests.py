...
class PaymentTestCase(APITestCase):
    fixtures = ['users.json', 'loans.json', 'payments.json']
    ...
    def test_retrieve_payment(self) -> None:
        payment = Payment.objects.filter(loan__client__username="johnlennon").first()

        response = self.client.get(self.reverse_url('payments-detail', pk=payment.pk))

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_retrieve_payment_another_client(self) -> None:
        payment = Payment.objects.filter(loan__client__username="paulmc").first()
